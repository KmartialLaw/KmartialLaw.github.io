...
export default class MissionManager1 extends BaseMissionManager {
    constructor(scene) {
        super(scene);
        this.missionCompleted = false;
        this.bombsDestroyed = 0;
        this.swatsDestroyed = 0;
        this.requiredDestroys = 100;
        this.firstConditionMet = false;
    }

    checkMissionCompletion() {
        if (this.firstConditionMet && this.missionCompleted) {
            this.completeMission('GameScene02');
        }
    }
    ...
    updateDestroyCount(type) {
        ...
        if (this.bombsDestroyed >= this.requiredDestroys && 
            this.swatsDestroyed >= this.requiredDestroys) {
